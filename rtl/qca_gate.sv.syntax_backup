...
module qca_gate #(
    parameter int QCA_WIDTH      = 4,    
    parameter int GATING_DEPTH   = 2,    
    ...
    input  logic [QCA_WIDTH-1:0]   qca_cmd_in,      
    input  logic                    qca_valid_in,    
    output logic                    qca_ready_in,    

    // QCA Gating Control
    input  logic                    gating_enable,   
    input  logic [QCA_WIDTH-1:0]   gating_mask,     
    input  logic [$clog2(GATING_DEPTH)-1:0] gating_depth_cfg,  
    ...
    output logic [QCA_WIDTH-1:0]   qca_cmd_out,     
    output logic                    qca_valid_out,   
    input  logic                    qca_ready_out,   

    // ca_distributor Integration
    output logic [QCA_WIDTH-1:0]   ca_cmd,          
    output logic                    ca_valid,        
    input  logic                    ca_ready,        

    // Error and Overflow Reporting
    output logic                    buffer_overflow, 
    output logic                    gating_error,    
    output logic [$clog2(BUFFER_DEPTH)-1:0] buffer_occupancy, 

    // Status and Debug
    output logic                    gating_active,   
    output logic                    cmd_blocked,     
